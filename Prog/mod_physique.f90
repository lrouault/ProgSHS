...
contains
  ...
  subroutine cd_limite()
    ...
    V = 0
    do j = 1,Ny
       V((j-1)*Nx+1) = V((j-1)*Nx+1) +  min(Tad, 2*Text+Tad*t)*(lambda*dt)/(dx**2)
       V(j*Nx) = V(j*Nx) + U(j*Nx)*(lambda*dt)/(dx**2)
    ...
    do i = 1,Nx
       V(i) =V(i) + U(i)*(lambda*dt)/(dy**2)
       V((Ny-1)*Nx+i) =V((Ny-1)*Nx+i) + U((Ny-1)*Nx+i)*(lambda*dt)/(dy**2)
       !   V(bij(i,1,Nx))  =- 1000  
